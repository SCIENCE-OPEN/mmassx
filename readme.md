...
[![Ask Me Anything !](https://img.shields.io/badge/Ask%20about-anything-1abc9c.svg)](https://github.com/SCIENCE-OPEN/mmassx/issues/new/choose)
...
- **HMDB database** was processed by these steps:
  ...
  ```
  - converted split `.xml` to one `.csv` using `$ cd datasets/HMDB` & `$ python split_xml_to_one_csv.py hmdb_metabolites_split_output hmdb_metabolites.csv`
...
- An executable file (e.g. `dist/mmassx-6.0.1.exe`) is generated by the script `$ build-mmassx.bat` which has to be executed on Windows or on Windows inside VirtualBox, administrator privileges are not required, either Miniconda or Anaconda has to be installed and initialized, e.g. execute the script inside Anaconda Prompt (the script is build around `$ pyinstaller --clean mmass.spec`)
