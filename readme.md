...
- An executable file (e.g. `dist/mmassx-6.0.1.exe`) is generated by the script `$ build-mmassx.bat` which has to be executed on Windows or on Windows inside virtualbox, in both cases, with either Miniconda or Anaconda installed (the script is build around `$ pyinstaller --clean mmass.spec`)
